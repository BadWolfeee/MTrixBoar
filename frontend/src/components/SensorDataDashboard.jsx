...
export default function SensorDataGraph({
  ...
  // Prepare data for Recharts (number x number)
  const prepared = useMemo(() => {
    const mapped = (data ?? [])
      .filter(d => d && d[timeKey] != null && d[valueKey] != null)
      .map(d => ({
        // 't' is milliseconds since epoch so we can use scale="time"
  ...
  const longFormat = spanMs > 24 * 60 * 60 * 1000; // > 24h
    ...
    <ResponsiveContainer width="100%" height={250}>
      <LineChart data={prepared} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
        ...
        <XAxis
          ...
          minTickGap={40}         // increase if you still see crowding
        />
        <YAxis
          width={48}
          tickMargin={6}
        ...
        <Line
          ...
          stroke={color}          // MUI palette color passed from parent
          strokeWidth={2}
        />
      </LineChart>
    </ResponsiveContainer>
